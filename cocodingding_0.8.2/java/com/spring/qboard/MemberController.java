...
public class MemberController {
	...
	public String userMain(
		...
		try {
			...
			// 파라미터 레벨이 0이면 최대 레벨을 표시, 그 외에는 지정된 레벨을 표시
			if (levels == 0) {
...
*/
		...
		for (MemberVO member: memberList) {
			if (member.getId().equals(memberVO.getId())){
				return HomeController.alert(model, "아이디 중복입니다. 다시 확인해주세요", "/qboard");
			} else if (member.getNickname().equals(memberVO.getNickname())) {
				return HomeController.alert(model, "닉네임 중복입니다. 다시 확인해주세요", "/qboard");
			} else if (member.getEmail().equals(memberVO.getEmail())) {
				return HomeController.alert(model, "이메일 중복입니다. 다시 확인해주세요", "/qboard");
			}
		}
	...
	public String idsearch(Model model, MemberVO memberVO, HttpSession session) throws Exception {
		...
		for(MemberVO member: memberList) {
			if (member.getEmail().equals(memberVO.getEmail())) {
				return HomeController.alert(model, "회원님의 ID는 [" + member.getId() + "] 입니다.", "/qboard");
			}
		}

		return HomeController.alert(model, "일치하는 회원 정보가 없습니다.", "/qboard");
	}
	...
	public String pwsearch(Model model, MemberVO memberVO, HttpSession session) throws Exception {
		...
		for(MemberVO member: memberList) {
			if (member.getId().equals(memberVO.getId()) && member.getEmail().equals(memberVO.getEmail())) {
				...
				return HomeController.alert(model, "새로운 비밀번호 [" + member.getPassword() + "] 이 설정되었습니다.로그인 후 비밀번호를 변경해주세요.", "/qboard");
			}
		}
