		...
		<div class="container-fluid">
			...
			<div class="collapse navbar-collapse" id="myNavbar">
				...
				<ul class="nav navbar-nav">
					...
					<li><a href="/qboard/incorrectList.ic">오답노트</a></li>
					...
					<div class="panel-body">
						...
						<table class="table">
							<c:if test="${testType eq '레벨 업 테스트'}">
								<tr>
									<td class="text-left" colspan="2">${userInfo.nickname} 님은 레벨 업에 ${success}하셨습니다.</td>
								</tr>
								<tr>
									<td class="text-left">
										<c:if test="${success eq '성공'}">
											${userInfo.nickname} 님은 현재 ${field} Lv.${levels} 입니다.
										</c:if>
										<c:if test="${success eq '실패'}">
											${userInfo.nickname} 님은 현재 ${field} Lv.${levels-1} 입니다.
										</c:if>
									</td>
									<td>프로그래밍 언어 ${step} 단계입니다.</td>
								</tr>
								<tr>
									<td colspan="2"></td>
								</tr>
							...
							<tr>
								<td class="text-left" colspan="2">진단일자 : ${date}</td>
							</tr>
							<tr>
								<th class="active col-xs-6 text-left">${userInfo.nickname} 님의 ${field} ${testType} 결과</th>
								<th class="success col-xs-6 text-left">코딩고 종합평가</th>
							</tr>
							<tr>
								<th class="text-left">총 ${total} 문항 중 ${correct} 문항을 맞추셨습니다.</th>
								...
								<table class="table table-striped table-bordered table-hover">
									...
									<tbody>
										<c:forEach var="article" items="${list}" begin="${pageVO.curStartNumber}" end="${pageVO.curEndNumber}" varStatus="status">
											<tr>
												<td <c:if test="${article.result ne article.check}">style="color:red;"</c:if>>${pageVO.curStartNumber + status.count}</td>
												<td <c:if test="${article.result ne article.check}">style="color:red;"</c:if>>${article.abbrev}</td>
												<td <c:if test="${article.result ne article.check}">style="color:red;"</c:if>>${article.result}</td>
												<td <c:if test="${article.result ne article.check}">style="color:red;"</c:if>>${article.check}</td>
											    <td><!-- 문제 보기 버튼 -->
													<button type="button" <c:if test="${article.result eq article.check}">class="btn btn-primary"</c:if> <c:if test="${article.result ne article.check}">class="btn btn-danger"</c:if> data-toggle="modal" data-target="#modQuiz${status.index}">
														상세보기
													</button>
													<!-- 문제 보기 버튼 끝 -->
													<!-- 문제 보기 모달 -->
