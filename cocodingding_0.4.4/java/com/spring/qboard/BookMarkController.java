...
public class BookMarkController {
	...
	public String boardList(@RequestParam(defaultValue="1") int page,
		...
		try {
			...
			for (BookMarkVO article: bookMarkList) {
				...
				Quizlist.get(bookMarkList.indexOf(article)).setId(article.getId());
			}
	...
	// 게시글 확인
	@RequestMapping(value = "/bookMarkView.bm/{Qnum}", method = RequestMethod.GET)
	public String boardView(@PathVariable int qnum, Model model, HttpSession session) throws Exception {
		try {
			model.addAttribute("userInfo", (MemberVO) session.getAttribute("userInfo"));
			
			BookMarkVO article = bookMarkService.selectBookMark(qnum);
