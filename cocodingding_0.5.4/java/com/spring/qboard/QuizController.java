		...
		try {
			...
			for (QuizVO quiz: testQuiz) {
				if (quiz.getCheck().equals(quiz.getResult())) {
					...
					newGrade[Integer.parseInt(quiz.getPart().substring(4))]++;
					correct++;
			...
			case 2: // 자동, 수동 문제일 때 현재 레벨에 점수가 추가됨
				...
				gradeVO.setPart1(gradeVO.getPart1()/2 + newGrade[0]/2);
				gradeVO.setPart2(gradeVO.getPart2()/2 + newGrade[1]/2);
				gradeVO.setPart3(gradeVO.getPart3()/2 + newGrade[2]/2);
				gradeVO.setPart4(gradeVO.getPart4()/2 + newGrade[3]/2);
				gradeVO.setPart5(gradeVO.getPart5()/2 + newGrade[4]/2);
				gradeVO.setPart6(gradeVO.getPart6()/2 + newGrade[5]/2);
