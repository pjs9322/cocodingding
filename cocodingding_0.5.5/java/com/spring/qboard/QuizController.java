	...
	public String complete(@PathVariable int code, @RequestParam(defaultValue="1") int page, Model model, HttpSession session) throws Exception {
		...
		try {
			...
			case 2: // 자동, 수동 문제일 때 현재 레벨에 점수가 추가됨
				...
				gradeVO.setPart1(gradeVO.getPart1()/2 + newGrade[0]);
				gradeVO.setPart2(gradeVO.getPart2()/2 + newGrade[1]);
				gradeVO.setPart3(gradeVO.getPart3()/2 + newGrade[2]);
				gradeVO.setPart4(gradeVO.getPart4()/2 + newGrade[3]);
				gradeVO.setPart5(gradeVO.getPart5()/2 + newGrade[4]);
				gradeVO.setPart6(gradeVO.getPart6()/2 + newGrade[5]);
