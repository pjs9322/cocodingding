...
<head>
    ...
    <style>
		...
		input.choice:checked ~ label { color:red; }
	    /* 라디오버튼 리스트 */
        ...
        <div class="row content">
			...
			<div class="panel panel-primary">
			    <div class="panel-heading">
			        <h2 class="panel-title text-left">테스트</h2>
			    </div>
			    <!-- 바디패널 -->
							...
							<table class="table">
										...
										<c:forEach var="item" items="${testQuiz}" end="8" varStatus="status">
											<c:choose>
												<c:when test="${code eq status.index}">
													<tr>
														<td class="text-center">
															<span style="color:red;">${status.count}</span>
														</td>
														<td class="text-center">
															<span style="color:red;">${item.check}</span>
														</td>
													</tr>	
												</c:when>
												<c:otherwise>
										...
										<c:forEach var="item" items="${testQuiz}" begin="${fn:length(testQuiz) - 9}" end="${fn:length(testQuiz)}" varStatus="status">
											<c:choose>
												<c:when test="${code eq status.index}">
													<tr>
														<td class="text-center">
															<span style="color:red;">${status.count + fn:length(testQuiz) - 9}</span>
														</td>
														<td class="text-center">
															<span style="color:red;">${item.check}</span>
														</td>
										...
										<c:forEach var="item" items="${testQuiz}" begin="${code - 4}" end="${code + 4}" varStatus="status">
											<c:choose>
												<c:when test="${code eq status.index}">
													<tr>
														<td class="text-center">
															<span style="color:red;">${status.count + code - 4}</span>
														</td>
														<td class="text-center">
															<span style="color:red;">${item.check}</span>
														</td>
								...
								<tr>
									<td colspan="4">
										...
										<label for="c1" onclick="goData(1);">
											<span style="cursor:pointer">${testQuiz.get(code).choice1}</span>
										</label>
									</td>
									
									<td class="success text-center" colspan="2" rowspan="4">
										<br><br>
										<span class="glyphicon glyphicon-question-sign" data-toggle="popover" title="이 문제를 북마크에 등록" data-content="북마크에 등록하시면 언제든지 이 문제를 확인할 수 있습니다."></span>
										<button type="button" class="btn btn-success btn-lg" onclick="location.href = '/qboard/bookMarkInsert.bm/${testType}/${testQuiz.get(code).field}/${testQuiz.get(code).levels}/${code}/${testQuiz.get(code).qnum}'">북마크 등록</button>
									</td>
								</tr>
								...
								<tr>
									<td colspan="4">
										...
										<label for="c2" onclick="goData(2);">
											<span style="cursor:pointer">${testQuiz.get(code).choice2}</span>
										</label>
									</td>
								</tr>
								...
								<tr>
									<td colspan="4">
										...
										<label for="c3" onclick="goData(3);">
											<span style="cursor:pointer">${testQuiz.get(code).choice3}</span>
										</label>
									</td>
								</tr>
								...
								<tr>
									<td colspan="4">
										...
										<label for="c4" onclick="goData(4);">
											<span style="cursor:pointer">${testQuiz.get(code).choice4}</span>
										</label>
									</td>
								</tr>
								...
								<tr>
									...
									<td colspan="2" class="text-center">
										<button type="button" class="btn btn-danger btn-lg" onClick="end_Quiz()">제출</button>
									</td>
								</tr>
							</table>
						</div>
