...
public class BoardController {
	...
	// 게시글 정렬을 위한 서브클래스
	class DescendingObj implements Comparator<BoardVO> {
	    @Override
	    public int compare(BoardVO o1, BoardVO o2) {
	    	Integer i1 = new Integer(o1.getBnum());
	    	Integer i2 = new Integer(o2.getBnum());
	...
	public String boardWriteForm(Model model, HttpSession session) throws Exception {
		try {
			...
			List<BookMarkVO> bookMarkList = bookMarkService.selectBookMark(userInfo.getId());
			List<IncorrectVO> incorrectList = incorrectService.selectIncorrect(userInfo.getId());
			
			for (BookMarkVO bookMark: bookMarkList) {
				quizList.add(bookMark);
			}
			
			boolean overlap = false;
			for (IncorrectVO incorrect: incorrectList) {
				for (QuizVO quiz: quizList) {
					if (quiz.getQnum() == incorrect.getQnum()) {
						overlap = true;
					}
				}
				if (overlap) {
				...
				quizList.add(incorrect);
			}
			
			// 문제 내용 추가
			List<QuizVO> list = new ArrayList<QuizVO>();
			for (QuizVO quiz: quizList) {
				list.add(quizService.selectQuiz(quiz.getQnum()));
			}
	...
	public String boardView(@PathVariable int bnum, Model model, HttpSession session) throws Exception {
		try {
			...
			QuizVO quiz = quizService.selectQuiz(article.getQnum());
	...
	public String boardModifyForm(@PathVariable int bnum, Model model, HttpSession session) throws Exception {
		try {
			...
			if (!userInfo.getId().equals(article.getId()) && userInfo.getAdmin() == 0) {
				return HomeController.alert(model, "권한이 없습니다.", "/qboard/boardView.bo/" + bnum);
			}
			...
			for (IncorrectVO incorrect: incorrectList) {
				for (QuizVO quizVO: quizList) {
					if (quizVO.getQnum() == incorrect.getQnum()) {
						overlap = true;
					}
				}
				if (overlap) {
			...
			List<QuizVO> list = new ArrayList<QuizVO>();
			for (QuizVO quizVO: quizList) {
				list.add(quizService.selectQuiz(quizVO.getQnum()));
			}
			
			for (QuizVO quizVO: list) {
				String abbrev = quizVO.getDocument();
				if (quizVO.getDocument().length() > 30) {
					abbrev = quizVO.getDocument().substring(0, 30);
				}
				quizVO.setAbbrev(abbrev);
			}
